    ...
    Plugin.prototype = {
        ...
        render: function () {
            ...
            for (var i = 0; i < pageCount; i++) {
                var boxliJq = $("<li>").addClass("sub").attr("id",'ul_inner_'+i).css({'width':(this.options.clientWidth) + "px", 'height':(this.options.clientHeight) + 'px'});
                boxliJq.text("第" + i + "页");
        ...
        next: function () {
            ...
            if (this.currentIndex > this.nodeOrders.length - 1) {
                this.currentIndex = 0;
                $("#ul_inner").removeClass('animate').css({ "transform": 'translateX(' + (-1 * this.currentIndex * this.options.clientWidth) + 'px)' });
            } else {
                if ($("#ul_inner").hasClass('animate')) {
                    $("#ul_inner").css({ "transform": 'translateX(' + (-1 * this.currentIndex * this.options.clientWidth) + 'px)' });
                } else {
                    $("#ul_inner").addClass('animate').css({ "transform": 'translateX(' + (-1 * this.currentIndex * this.options.clientWidth) + 'px)' });
                }
            }
        },
        prev: function () {
